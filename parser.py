...
MINUS_WORDS = [
    # ❌ ЭСТЕТИКА, АППАРАТКА И ТЕЛО (НЕ колют филлеры)
    "эстет", "эстетист", "эстетика", "аппаратная", "аппаратный", "аппаратка", "аппаратный массаж",
    "массаж", "массажист", "чистка лица", "smas", "смас", "lpg", "rf-лифтинг", "рф-лифтинг", 
    ...
    # ❌ ЧИСТАЯ ДЕРМАТОЛОГИЯ, БОЛЕЗНИ И ИНСТРУМЕНТЫ (Медицина, а не бьюти)
...
Твоя задача: отличить ПРОФЕССИОНАЛА (наш клиент) от ПАЦИЕНТА (мусор). Никаких баллов. Только бизнес-статус.

КАТЕГОРИИ КЛИЕНТОВ (БЕРЕМ):
1. HOT (Горячий): Клиника, врач, инъекционист. Ищет поставщиков, запрашивает прайсы, закупает препараты (филлеры, ботокс).
2. WARM (Теплый): Частный мастер. Обсуждает техники, делится опытом, спрашивает советы у коллег.

МУСОР (НЕ БЕРЕМ):
- TRASH: Пациенты ("хочу сделать губы"), эстетисты, мастера маникюра, спамеры.
...
async def check_pulse():
    if S.leads_session_total > 0 and S.leads_session_total % 50 == 0:
        msg = (
            f"💓 **ПУЛЬС ПАРСЕРА** 💓\n\n"
...
def register_handlers(bot):
    ...
    async def _(e):
        ...
        await e.reply("🚀 МНОГОПОТОК ЗАПУЩЕН! Пульс настроен на 50 лидов.", buttons=get_keyboard())
    ...
    async def _(e):
        ...
        await e.reply("🛑 ПАРСЕР ОСТАНОВЛЕН. Очередь групп сброшена. База и закладки сохранены.", buttons=get_keyboard())
...
async def main():
    ...
    try:
        await S.bot.send_message(ADMIN_ID, "🔧 Терминал парсера обновлен (Словарь дерматологии). Готов к работе.", buttons=get_keyboard())
    except:
        pass
