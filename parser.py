...
ADMIN_ID = 1568924415
OPENAI_API_KEY = "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 " # <--- ТВОЙ КЛЮЧ ОТ OPENAI
...
# ══════════════════════════════════════════════════════════════
# УМНЫЙ МОЗГ ИИ (БЕЗ ЦИФР, ТОЛЬКО БИЗНЕС-ЛОГИКА)
# ══════════════════════════════════════════════════════════════
...
Ответь строго в JSON:
{
  ...
  "category": "HOT",  // или WARM, или TRASH
}"""
...
async def check_pulse():
    """Отправляет статистику каждые 20 лидов"""
...
async def process_user(client, user_obj, messages, group_link, acc_name):
    ...
    # Сначала жесткий фильтр
    category, reason = hard_filter(full_content, username)
    
    # Если жесткий фильтр не уверен, подключаем мозги ИИ
    if category is None:
    ...
    if category in ['HOT', 'WARM']:
        with sqlite3.connect(DB_PATH) as conn:
            try:
                ...
                # Проверяем, не пора ли бить пульс (каждые 20)
                await check_pulse()
                
            except sqlite3.IntegrityError:
                pass # Уже есть в базе
...
async def account_worker(name, session_path, proxy):
    ...
    while not S.stop_event.is_set():
        try:
            ...
            async for msg in client.iter_messages(entity, limit=BATCH_SIZE, offset_id=0):
...
async def export_txt(event):
    ...
    with open(path, "w", encoding="utf-8") as f:
        for r in rows:
            ...
            # Формат: [HOT/WARM] Contact | Name | Text
...
def register_handlers(bot):
    ...
    async def _(e):
        ...
        await e.reply("🚀 МНОГОПОТОК ЗАПУЩЕН! Жди ПУЛЬС каждые 20 лидов.", buttons=get_keyboard())
    ...
    async def _(e):
        S.waiting_for_links = False
        with sqlite3.connect(DB_PATH) as conn:
            conn.executescript("DELETE FROM leads; DELETE FROM seen;")
        while not S.queue.empty():
            S.queue.get_nowait()
            S.queue.task_done()
        # Сбрасываем счетчики пульса
        S.leads_session_total = 0
        S.leads_hot = 0
        S.leads_warm = 0
        await e.reply("♻️ БАЗА ОБНУЛЕНА, счетчики пульса сброшены.", buttons=get_keyboard())
